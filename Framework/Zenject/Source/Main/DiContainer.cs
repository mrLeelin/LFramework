...
namespace Zenject
{
    ...
    public class DiContainer : IInstantiator
    {
...
#if !NOT_UNITY3D
                _inheritedDefaultParent = parentContainers[0].DefaultParent;
#endif

                // Make sure to avoid duplicates which could happen if a parent container
                // appears multiple times in the inheritance chain
                var seenAncestors = new HashSet<DiContainer>();
                for (int i = 0; i < ancestorContainers.Length; i++)
                {
                    var ancestorContainer = ancestorContainers[i];
                    if (!seenAncestors.Add(ancestorContainer))
                    {
                        continue;
                    }
        ...
        public IEnumerable<IProvider> AllProviders
        {
            get
            {
                var set = new HashSet<IProvider>();
                foreach (var providerList in _providers.Values)
                {
                    for (int i = 0; i < providerList.Count; i++)
                    {
                        var provider = providerList[i].Provider;
                        if (set.Add(provider))
                        {
        ...
        public void InjectExplicit(object injectable, List<TypeValuePair> extraArgs)
        {
            ...
            //Exist NotInject attribute
            if (ZenUtilInternal.IsExitNotInjectAttribute(injectableType))
            {
                return;
            }
