...
{
    [CreateAssetMenu(fileName = "SpriteCollection", menuName = "LFramework/SpriteCollection", order = 0)]
    public class SpriteCollection : ScriptableObject
    {
        [SerializeField]
        private SerializableDictionary<string, Sprite> m_Sprites = new SerializableDictionary<string, Sprite>();
        ...
        public List<string> GetSpriteName()
        {
            return m_Sprites.Keys.ToList();
        }
        ...
        void HandlePackable(Object obj)
        {
            ...
            string name = GetSpriteName(path);
            if (obj is Sprite sp)
            {
                ...
                else
                {
                    ...
                    m_Sprites[GetSpriteName(regularPath)] = sp;
                }
            }
            else if (obj is Texture2D)
            {
                ...
                if (objects.Length == 2)
                {
                    m_Sprites[name] = GetSprites(objects)[0];
                }
                else
                {
                    ...
                    for (int j = 0; j < sprites.Length; j++)
                    {
                        ...
                        m_Sprites[GetSpriteName(regularPath)] = sprites[j];
                    }
                }
            }
            else if (obj is DefaultAsset && ProjectWindowUtil.IsFolder(obj.GetInstanceID()))
            {
                string[] files = Directory.GetFiles(path, "*.*", SearchOption.AllDirectories)
                    .Where(_ => !_.EndsWith(".meta") && (_.EndsWith(".png") || _.EndsWith(".jpg"))).Select(Utility.Path.GetRegularPath).ToArray();
                foreach (string file in files)
                {
                    Object[] objects = AssetDatabase.LoadAllAssetsAtPath(file);
                    if (objects.Length == 2)
                    {
                        m_Sprites[GetSpriteName(file)] = GetSprites(objects)[0];
                    }
                    else
                    {
                        Sprite[] sprites = GetSprites(objects);
        ...
        private string GetSpriteName(string spritePath)
        {
