...
namespace LFramework.Runtime
{
    ...
    public class GameNotificationsComponent : GameFrameworkComponent
    {
        ...
        public override void UpdateComponent(float elapseSeconds, float realElapseSeconds)
        {
            ...
            for (int i = PendingNotifications.Count - 1; i >= 0; --i)
            {
                ...
                if (time < DateTime.Now)
                {
                    ...
                    EventComponent.Fire(this, LocalNotificationExpiredEventArg.Create(queuedNotification));
                    Log.Info("[GameNotifications] Notification expired: {0}", queuedNotification.ToString());
                }
            }
        }
        ...
        public void Initialize(Action initializeCallBack)
        {
        ...
        public IEnumerator Initialize()
        {
            if (Initialized)
            {
                Log.Error("NotificationsManager already initialized.");
                _initializeCallBack?.Invoke();
                yield break;
            }
            ...
            Platform = new GameNotificationsPlatform(args, this);
        ...
        {
            Log.Info("[GameNotifications] Notification received: {0}  inForeGround : {1}",
                deliveredNotification.ToString(), inForeground);
            ...
            if (deliveredIndex >= 0)
            {
                ...
                EventComponent.Fire(this,
                    LocalNotificationDeliveredEventArg.Create(PendingNotifications[deliveredIndex]));
