...
namespace UniRx
{
    ...
    {
        IReactiveProperty<bool> CanExecute { get; }
    ...
    public class ReactiveCommand : ReactiveCommand<Unit>
    {
        ...
        public ReactiveCommand(bool initialValue = true)
            : base(initialValue)
        {
        }
    ...
    public class ReactiveCommand<T> : IReactiveCommand<T>, IDisposable
    {
        ...
        public IReactiveProperty<bool> CanExecute
        {
            get
            {
                return canExecute;
    ...
    public static class ReactiveCommandExtensions
    {
        ...
        /// </summary>
        public static IDisposable BindTo(this IReactiveCommand<Unit> command, UnityEngine.Events.UnityEvent unityEvent)
        {
            var d2 = unityEvent.AsObservable().SubscribeWithState(command, (x, c) => c.Execute(x));
            return d2;
        }
        /// <summary>
        /// Bind ReactiveCommand to button's interactable and onClick.
        /// </summary>
        public static IDisposable BindTo<T>(this IReactiveCommand<T> command, UnityEngine.Events.UnityEvent<T> unityEvent)
        {
            var d2 = unityEvent.AsObservable().SubscribeWithState(command, (x, c) => c.Execute(x));
            return d2;
        }
        ...
        /// Bind ReactiveCommand to button's interactable and onClick.
        /// </summary>
        public static IDisposable BindToOnClick(this IReactiveCommand<Unit> command, UnityEngine.Events.UnityEvent unityEvent,Action<Unit> onClick)
        {
            var d2 = unityEvent.AsObservable().SubscribeWithState(command, (x, c) => c.Execute(x));
            var d3 = command.Subscribe(onClick);
            return StableCompositeDisposable.Create(d2, d3);;
        }
        
        /// Bind ReactiveCommand to button's interactable and onClick.
        /// </summary>
        public static IDisposable BindToOnClick<T>(this IReactiveCommand<T> command, UnityEngine.Events.UnityEvent<T> unityEvent,Action<T> onClick)
        {
            var d2 = unityEvent.AsObservable().SubscribeWithState(command, (x, c) => c.Execute(x));
            var d3 = command.Subscribe(onClick);
            return StableCompositeDisposable.Create(d2, d3);;
