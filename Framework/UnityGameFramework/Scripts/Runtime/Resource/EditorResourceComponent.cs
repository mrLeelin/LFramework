...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class EditorResourceComponent : GameFrameworkComponent, IResourceManager
    {
...
#pragma warning restore 0067, 0414
        ...
        public override void AwakeComponent()
        {
            ...
            {
                Log.Error("Can not find resource component.");
                return;
            }
            if (resourceComponent.ResourceMode == ResourceMode.Addressable)
            {
                baseComponent.EditorResourceHelper = GameFrameworkEntry.GetModule<IResourceManager>();
            }
            else
            {
                ...
                {
                    baseComponent.EditorResourceHelper = null;
                }
            }
        }
        ...
        public override void UpdateComponent(float elapseSeconds, float realElapseSeconds)
        {
             ...
             if (m_LoadAssetInfos.Count > 0)
            {
                ...
                while (current != null && count < m_LoadAssetCountPerFrame)
                {
                    ...
                    float seconds = (float)(DateTime.UtcNow - loadAssetInfo.StartTime).TotalSeconds;
                    if (seconds >= loadAssetInfo.DelaySeconds)
                    {
                        ...
                        if (asset != null)
                        {
                            ...
                            {
                                loadAssetInfo.LoadAssetCallbacks.LoadAssetSuccessCallback(loadAssetInfo.AssetName, asset, seconds, loadAssetInfo.UserData);
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            loadAssetInfo.LoadAssetCallbacks.LoadAssetUpdateCallback(loadAssetInfo.AssetName, seconds / loadAssetInfo.DelaySeconds, loadAssetInfo.UserData);
                        }
        ...
        public void InstantiateAsset(string assetName, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            //Ignore
        }
