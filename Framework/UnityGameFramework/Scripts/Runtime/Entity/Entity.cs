...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class Entity : MonoBehaviour, IEntity
    {
        ...
        public void OnInit(int entityId, string entityAssetName, IEntityGroup entityGroup, bool isNewInstance,
            IEntityHelper entityHelper, object userData)
        {
            m_Id = entityId;
            ...
            {
                if (entityLogicType.IsInstanceOfType(m_EntityLogic))
                {
                ...
                DestroyImmediate(m_EntityLogic);
                m_EntityLogic = null;
            }
            ...
            entityHelper.BeforeOnInitEntity(this.gameObject);
            try
            {
        ...
        public void OnRelease()
        {
            ...
            {
                Log.Error("Entity '[{0}]{1}' OnRelease with exception '{2}'.", m_Id, m_EntityAssetName, exception);
            }
        }
        ...
        public T FindLogic<T>() where T : EntityLogic
        {
            if (Logic == null)
            {
                return null;
            ...
            if (Logic is not T)
            {
                Log.Fatal($"Error logic type '{typeof(T)}'. Logic Type '{Logic.GetType()}'");
            }
            var result = Logic as T;
            if (result == null)
            {
                Log.Fatal($"Error logic type '{typeof(T)}'. Logic Type '{Logic.GetType()}'");
            }
        ...
        public bool FindLogic<T>(out T logic) where T : EntityLogic
        {
            logic = null;
            if (Logic == null)
            {
